...
extension TopicsViewController {
    func showSortAlert() {
        let ac = UIAlertController(title: "Choose sort option",
                                   message: "",
                                   preferredStyle: .actionSheet)
        
        let createdDateAction = UIAlertAction(title: "Created date",
                                              style: .default) { [weak self] _ in
            self?.topicViewModels.sort(by: {$0.topic.created_on ?? 0 < $1.topic.created_on ?? 0})
            self?.tableView.reloadData()
        }
        
        let dueDateAction = UIAlertAction(title: "Due date",
                                          style: .default) { [weak self] _ in
            self?.topicViewModels.sort(by: {$0.topic.due_date ?? 0 < $1.topic.due_date ?? 0})
            self?.tableView.reloadData()
        }
        
        let statusAction = UIAlertAction(title: "Status",
                                         style: .default) { [weak self] _ in
